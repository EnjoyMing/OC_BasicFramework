...
typedef NS_ENUM(NSUInteger, SNViewLayoutItemDirection) {
    SNViewLayoutItemLeft,
    SNViewLayoutItemCenter,
    SNViewLayoutItemRight,
};


@interface SNViewLayout () {
    struct {
...
@interface SNLayoutObject ()
@property (nonatomic, weak) UIView *pageView;
@end

@implementation SNViewLayout
- (instancetype)init {
    if (self = [super init]) {
...
- (void)setDelegate:(id<SNViewLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(SNLayoutObject *)layout {
    _layout = layout;
...
- (SNViewLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    SNViewLayoutItemDirection direction= SNViewLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = SNViewLayoutItemCenter;
    }else if (centerX - contentCenterX < 0) {
        direction = SNViewLayoutItemLeft;
    }
    return direction;
}
...
{
    return _layout.layoutType == SNViewLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != SNViewLayoutNormal) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if(_layout.layoutType != SNViewLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(SNViewLayoutType)layoutType {
    switch (layoutType) {
        case SNViewLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case SNViewLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(SNViewLayoutType)layoutType {
    switch (layoutType) {
        case SNViewLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        SNViewLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case SNViewLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case SNViewLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
