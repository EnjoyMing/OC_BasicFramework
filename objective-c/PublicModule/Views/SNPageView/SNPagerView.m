...
#import "SNPagerView.h"

#define kPagerViewMaxSectionCount 200
#define kPagerViewMinSectionCount 18

NS_INLINE BOOL SNEqualIndexSection(SNIndexSection indexSection1,SNIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE SNIndexSection SNMakeIndexSection(NSInteger index, NSInteger section) {
    SNIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface SNPagerView()
<
UICollectionViewDataSource,
UICollectionViewDelegateFlowLayout,
SNViewLayoutDelegate
>
{
    struct {
...
@property (nonatomic, assign) SNIndexSection beginDragIndexSection;
...
- (void)addCollectionView {
    SNViewLayout *layout = [[SNViewLayout alloc]init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:SNPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (SNLayoutObject *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<SNPagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((SNViewLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<SNPagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(SNPagerScrollDirection)direction animate:(BOOL)animate {
    SNIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    ...
    if (!_isInfiniteLoop) {
        [self scrollToItemAtIndexSection:SNMakeIndexSection(index, 0) animate:animate];
        return;
    }

    [self scrollToItemAtIndexSection:SNMakeIndexSection(index, index >= self.curIndex ? _indexSection.section : _indexSection.section+1) animate:animate];
}
...
- (void)updateLayout {
    ...
    ((SNViewLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(SNIndexSection)indexSection {
...
- (SNIndexSection)nearlyIndexPathAtDirection:(SNPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (SNIndexSection)nearlyIndexPathForIndexSection:(SNIndexSection)indexSection direction:(SNPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == SNPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? SNMakeIndexSection(0, 0) : indexSection;
        } else if (direction == SNPagerScrollDirectionRight) {
            return SNMakeIndexSection(indexSection.index+1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? SNMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return SNMakeIndexSection(indexSection.index-1, 0);
    }
    
    if (direction == SNPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems-1) {
            return SNMakeIndexSection(indexSection.index+1, indexSection.section);
        }
        if (indexSection.section >= kPagerViewMaxSectionCount-1) {
            return SNMakeIndexSection(indexSection.index, kPagerViewMaxSectionCount-1);
        }
        return SNMakeIndexSection(0, indexSection.section+1);
    }
    ...
    if (indexSection.section <= 0) {
        return SNMakeIndexSection(indexSection.index, 0);
    }
    return SNMakeIndexSection(_numberOfItems-1, indexSection.section-1);
}

- (SNIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return SNMakeIndexSection(0, 0);
    }
    ...
    return SNMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(SNIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetPagerViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:SNMakeIndexSection(index, _isInfiniteLoop ? kPagerViewMaxSectionCount/3 : 0) animate:NO];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_delegate respondsToSelector:@selector(pagerView:didSelectedItemCell:atIndexSection:)]) {
        [_delegate pagerView:self didSelectedItemCell:cell atIndexSection:SNMakeIndexSection(indexPath.item, indexPath.section)];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    SNIndexSection newIndexSection =  [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    SNIndexSection indexSection = _indexSection;
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !SNEqualIndexSection(_indexSection, indexSection)) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if (fabs(velocity.x) < 0.35 || !SNEqualIndexSection(_beginDragIndexSection, _indexSection)) {
        targetContentOffset->x = [self caculateOffsetXAtIndexSection:_indexSection];
        return;
    }
    SNPagerScrollDirection direction = SNPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = SNPagerScrollDirectionLeft;
    }
    SNIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
- (void)pagerViewTransformLayout:(SNViewLayout *)pagerViewTransformLayout initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes {
    if (_delegateFlags.initializeTransformAttributes) {
        [_delegate pagerView:self initializeTransformAttributes:attributes];
    }
}

- (void)pagerViewTransformLayout:(SNViewLayout *)pagerViewTransformLayout applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes {
...
- (void)dealloc {
    ((SNViewLayout *)_collectionView.collectionViewLayout).delegate = nil;
